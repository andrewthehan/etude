...
public final class ImmutablePrioritySet<E> extends TreeSet<E>{
  private ImmutablePrioritySet(List<? extends E> list){
    // calling super(list) uses TreeSet#addAll which is not supported in ImmutablePrioritySet
    super((a, b) -> Integer.compare(list.indexOf(a), list.indexOf(b)));
    // calling super.addAll(c) uses TreeSet#add which is not supported in ImmutablePrioritySet
    list.forEach(super::add);
  }

  public static final ImmutablePrioritySet<Integer> of(int[] array){
    return new ImmutablePrioritySet<Integer>(Arrays.asList(ArrayUtil.boxed(array)));
  }

  public static final ImmutablePrioritySet<Double> of(double[] array){
    return new ImmutablePrioritySet<Double>(Arrays.asList(ArrayUtil.boxed(array)));
  }

  public static final ImmutablePrioritySet<Long> of(long[] array){
    return new ImmutablePrioritySet<Long>(Arrays.asList(ArrayUtil.boxed(array)));
  }
